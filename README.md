...
Locally it always lands on `/en/`. On the real site the root reads the browser's `Accept-Language` header and sends the reader to their own language instead. That is done by two Cloudflare redirect rules rather than by a file, so a local server never runs them. Their text lives in [`cloudflare/redirect-rules.md`](cloudflare/redirect-rules.md).
