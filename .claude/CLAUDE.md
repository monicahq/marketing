...
- **Only one prefetcher.** Turbo 8 prefetches on hover by default; `<meta name="turbo-prefetch" content="false">` in the layout turns that off and leaves it to instant.page, which backs off on slow connections and save-data, and falls back to mousedown on touch.
